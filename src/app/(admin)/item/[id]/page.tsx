...
import { Item } from '../../../../../actions/(admin)/getItemData';
  ...
  const handleDelete = async () => {
    if (deleteId) {
      try {
        ...
        // Get product data before deleting (to retrieve companyName and productName)
        ...
        setOpen(false);
        router.push(`/admin/${productData?.companyName}`);
  
        // Send a message about the deletion to the messages collection
        if (productData) {
          const messageRef = collection(db, "messages");
          await addDoc(messageRef, {
            image1: productData.image1, // Use the product's image1
            companyName: productData.companyName,
            productName: productData.productName,
            productId: deleteId,  // Store product ID
            createdAt: new Date().toISOString(),
            message: `${productData.companyName} has deleted the product: ${productData.productName}`,
            isClickable: false, // Mark message as non-clickable
